...
// Route pour le sondage HTML
app.get('/', (req, res) => {
  res.sendFile(path.join(__dirname, 'survey.html'));
});

// API route pour envoyer les réponses
app.post('/api/send-survey', async (req, res) => {
  try {
    const { userEmail, answers } = req.body;

    if (!userEmail || !answers) {
      return res.status(400).json({ error: 'Email et réponses requis' });
    }
...
app.listen(PORT, () => {
  console.log(`
╔═══════════════════════════════════════════════════════════╗
║          🎉 PSY SURVEY SERVER STARTED                     ║
╠═══════════════════════════════════════════════════════════╣
║                                                           ║
║  📍 URL: http://localhost:${PORT}                              ║
║                                                           ║
║  ✅ Prêt à recevoir les réponses!                        ║
║  📧 Emails envoyés à: germain.ia17@gmail.com            ║
║                                                           ║
